...
class EndoNeRF_Dataset(object):
    ...
    def get_init_pts(self, sampling='random'):
        if self.mode == 'binocular':
            ...
            print(f"深度图形状: {depth.shape}")  # 预期为 (H, W) 或 (H, W, 1)‌:ml-citation{ref="5,7" data="citationList"}
            print(f"深度图数据类型: {depth.dtype}")  # 应为浮点型（如 float32）
            print(f"深度值范围: min={depth.min()}, max={depth.max()}")  # 合理范围如 0~1 或 0~255‌:ml-citation{ref="1,5" data="citationList"}
    ...
    def get_pts_cam(self, depth, mask, color, disable_mask=False):
        ...
        Z = depth
        # print(f"Z 的形状: {Z.shape}")          # 预期输出 (512, 640)
        # print(f"X_Z 的形状: {X_Z.shape}")      # 预期输出 (512, 640)
        # print(f"Y_Z 的形状: {Y_Z.shape}")      # 预期输出 (512, 640)
        X, Y = X_Z * Z, Y_Z * Z
